-- Closed Funnel: stage transitions in the window AND lead createdtime also in the window.
-- Substitute {{START_DATE}} / {{END_DATE}} with IST calendar dates, e.g. DATE '2026-08-10'.
-- LPV is NOT included here -- it uses the same figure as Open Funnel (see open_funnel_query.sql),
-- since no lead exists yet at that stage and the same-day-creation restriction is inapplicable.
WITH day_rows AS (
    SELECT
