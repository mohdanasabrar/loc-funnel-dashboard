...
**Incident (2026-08-24): longest Trino saturation to date, plus a new variant of the Pulse-lag gotcha hitting 00_LPV itself, plus a major unexplained LPV volume surge.** Run unattended overnight per PM request ("refresh the LOC daily dashboard... make sure you update the entire platform... I am going to sleep"). Three distinct findings:

1. **Pulse lag hit `00_LPV` directly, not just the 4 extension sections.** `hive.workflow_nexus.lead_history_snapshot_v3` was fully current through 2026-08-23, but `screenviewagg_snapshot_v3` (which `open_funnel_query.sql`'s `lpv_matched` CTE reads `00_LPV` from) was only current through 2026-08-22. Every prior Pulse-lag incident (08-13, 08-17, etc.) described the lag as isolated to PAN Prefill Rate / Prefill Source / Campaign Traffic / field-validation — this is the first time it's been traced to `00_LPV` itself, which uses the *same* lagging table. Anchored the entire refresh (core funnel + all sections) at 2026-08-22 instead of "yesterday" (08-23) to avoid reporting a fabricated near-zero LPV. **Lesson: check `screenviewagg_snapshot_v3`'s lag BEFORE running the core Open/Closed Funnel queries, not just before the 4 extension sections — 00_LPV is not immune.**
2. **Trino saturation ran ~32 minutes, then a second ~15+ minute window after a brief recovery — the longest logged for this dashboard (prior worst: "over 30 minutes" on 08-14).** Did not retry in a tight loop at any point; used the wait productively on the pure-ES backend_errors section. As a direct result, PAN Prefill Rate MTD, Prefill Source Breakdown MTD, the entire Campaign-wise LPV→Lead Creation section (day + trend), Landing-Page field-validation (Trino half), and 2026-08-21's stage-04/07 detail (needed for that day's AI Funnel Trend point) could not be completed and carry forward stale with explicit per-section `stale_note`s — see `dashboard-data.json`.
3. **Major finding, cause not yet identified: LPV surged +104% in 2 days (358,574 on 08-20 → 578,672 on 08-21 → 731,170 on 08-22), isolated entirely to the `consumer-app/paytm-loc/lead-not-present-v3` screen** (raw distinct-customer counts on that one screen: 324,259 → 336,790 → 546,030 → 690,488 across 08-19→08-22; every other LPV-qualifying screen stayed flat). Lead Created grew far less proportionally, compressing LPV→Lead-Created conversion from 24.5% to 20.8% over the same window — real, but under anomaly-methodology.md's formal 15pp/20%-relative threshold (that rule targets ratio noise, not a volume-side shift this large). Filed as the top recommendation in the dashboard; root cause (real growth vs. bot/crawler vs. duplicate-tracking bug) needs a raw event-level sample (device/session dedup) plus the campaign-attribution breakdown, neither of which could be pulled this refresh due to the Trino incident above.

**Incident (2026-08-25): a resolved root-cause finding didn't propagate to 3 other data structures in the same JSON.** The 2026-08-24 refresh root-caused the Aug 21-23 LPV-surge/conversion-compression episode to the `postpaid_acq_Dailypush` push campaign (see `recommendations[0]` / `campaign_traffic.pattern_findings`) in a later same-day targeted pass -- but `core_funnel_stale_note`, `known_limitations`, and `ai_funnel_trend.insight_cards[0]` were never updated to match, and kept describing the cause as "not yet identified" for a full refresh cycle. Caught and fixed 2026-08-25. **Lesson: when a finding resolves mid-refresh, grep the whole JSON for its old "unresolved" phrasing (e.g. "not yet identified", "cause not identified") before finishing -- a resolution written into one field is not automatically consistent with every other field that described the same open question.** Separately, Trino saturation recurred for ~30 minutes straight (matching the 08-24 incident's length) -- core funnel + PAN Prefill Rate day completed in an early window, everything else (PAN Prefill MTD, Prefill Source Breakdown, Campaign-wise LPV->Lead Creation including the Goldbase/Hinglish targeted re-check, Landing-Page field-validation, the 08-21 full-stage backfill) stayed blocked and carries forward stale, now the longest-stale run for several of these sections. Also: publishing to BOTH artifact URLs (primary and backup) was blocked this refresh by a "this session hasn't viewed the latest version of the artifact" conflict guard that a WebFetch of the URL did not clear (the fetched content was a generic wrapper page, not the real artifact HTML) -- did not force-overwrite without explicit authorization; `dashboard.html` (canonical local file) and `dashboard-v2.html`/`dashboard-v3.html` (republish-ready copies) are fully updated and verified on disk, just not yet live at either URL as of this refresh.
